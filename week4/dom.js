...
function renderTasks(tasks) {
  ...
  const thelist = document.getElementById("todoList");

  // loop through the tasks array. transform (map) each task object into the appropriate HTML to represent a to-do.
  let allthelistitems = "";
  
  tasks.forEach(task => {
      allthelistitems += `<li ${task.completed ? 'class="strike"' : ""}>
    <p>${task.detail}</p>
    <div>
  ...
  thelist.innerHTML = allthelistitems;
}
...
function newTask() {
  ...
  let thetask = { detail: todo.value, completed: false}
  tasks.push(thetask);
...
function manageTasks(event) {
  ...
  const theaction = event.target.dataset.function;
  if(theaction == "delete")
  {
    removeTask(event.target.closest("li"));
  }
  else if(theaction == "complete")
  {
